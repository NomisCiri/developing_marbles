...
#library(rstan)
#library(tidybayes)
pacman::p_load(cowplot,brms,tidyverse,tidybayes)
...
MarbleData<-MarbleData#%>%filter((probGamble!=0.675) | Social1Ind0==0)
...
#MarbleData$PubertyStage=as.factor(MarbleData$PubertyStage)
#MarbleData=MarbleData[!is.na(MarbleData$PubertyStage),]
```
...
for(i in c(1,2,3,4)){
  ...
  AllModels[[i]]%>%tidybayes::spread_draws(alpha_add[1:NSubs],
                                           rho[1:NSubs],
                                           UncertaintyMulti_Risk_risk[1:NSubs],
                                           UncertaintyMulti_risk[1:NSubs],
                                           UncertaintyMulti_Risk_safe[1:NSubs],
                                           UncertaintyMulti_safe[1:NSubs]
  )%>%
  ...
  # tidybayes::spread_draws(alpha_add[1:NSubs],UncertaintyMulti[1:NSubs],UncertaintyMulti_Risk[1:NSubs],rho[1:NSubs])%>%mutate(subject=subs[as.numeric(`1:NSubs`)])%>%
  # pivot_longer(names_to = "parameter",values_to = "estimate",cols = c(alpha_add,UncertaintyMulti,UncertaintyMulti_Risk,rho))%>%
  # bind_rows(GeneralBetaEstimates_riskSafe_multi)->GeneralBetaEstimates_riskSafe_multi
}
...
# #ggsave(filename ="X_Figures/RawData.png",plot=Behavior,height = 7,width=10)
 ggsave(filename ="X_Figures/RawData.png",plot=plotPredsLeg,height = 7,width=10,dpi=320)
...
marbleDummy%>%
  ...
  ggplot(aes(x=age,y=value,color=Agegroup))+
  stat_summary(geom="bar",position=position_dodge(0.99),fill="white")+
  #geom_smooth(color="black",method="lm")+
  #stat_summary(aes(group=subject),geom="point",position=position_dodge(0.99),fill="white")+
  stat_summary(position=position_dodge(0.99))+
  facet_grid(SocialRiskSafe~DFE1DFD0,labeller = labeller(halfs=c("1"="1/4","2"="2/4","3"="3/4","4"="4/4"), DFE1DFD0=c("0"="Risk","1"="Uncertainty")))+
  theme_minimal()
...
marbleDummy%>%
  ...
  mutate(Solo=mean(Solo,na.rm=T))%>%rowwise()%>%
  mutate(diffRisk=Risk-Solo,
         diffSafe=Solo-Safe)%>%ungroup()%>%pivot_longer(names_to = "SocialRiskSafe",cols=c(diffRisk,diffSafe))%>%mutate(Dummy=case_when(SocialRiskSafe=="diffRisk"~"1",
                                                                                                                                        SocialRiskSafe=="diffSafe"~"2")
         )->socialchange

brm(formula=value~Dummy*poly(age,2), data=socialchange)
```
...
MarbleData%>%filter(DFE1DFD0==1)%>%group_by(subject)%>%mutate(
  Error=((as.numeric(PercentBlueEstimate) - as.numeric(as.character(PercentBlueShownRel))*100))
...
<!-- sjPlot::tab_model(model, show.ci50 = TRUE,digits = 2,transform=NULL)->what -->
<!-- ``` -->
...
summarized%>%filter(parameter=="sigma_sq_riskRisk" | parameter=="sigma_sq_riskUnc")%>%
  ggplot(aes(x=age,y=posteriorMean,color=parameter))+geom_jitter()+
  ...
  scale_color_manual(name=NA,
                     ...
                     labels=c("Described Risks","Expierienced Risks"),values = wes_palette("Royal1"))+
...
summarized%>%filter(parameter=="rho")%>%ggplot(aes(x=as.numeric(age),y=posteriorMean))+geom_jitter(color=wes_palette("Royal1")[4])+
...
summarized%>%filter(str_detect(parameter,"_risk"))%>%filter(!str_detect(parameter,"alpha") & !str_detect(parameter,"sigma"))%>%
  ggplot(aes(x=as.numeric(age),y=posteriorMean,color=parameter))+geom_jitter()+
  stat_summary(aes(fill=parameter),color="black",fun.data="mean_cl_boot",geom="pointrange",position=position_dodge(0.95),shape=21)+
  geom_hline(yintercept = 1,size=1,linetype="dotted")+
  #scale_x_continuous(name="Adolescence Stage",breaks = breaks,labels=labels2)+
  scale_color_manual(name="",
                     breaks=c("UncertaintyMulti_risk","UncertaintyMulti_Risk_risk"),
...
summarized%>%filter(str_detect(parameter,"_safe"))%>%filter(!str_detect(parameter,"alpha"))%>%
  ...
  #scale_x_continuous(name="Adolescence Stage",breaks = breaks,labels=labels2)+
  scale_color_manual(name="",
                     breaks=c("UncertaintyMulti_safe","UncertaintyMulti_Risk_safe"),
                     labels=c("Uncertainty","Risk"),values = c(wes_palette("Royal1")[2],wes_palette("Royal1")[1]))+
  scale_fill_manual(name="",
                    breaks=c("UncertaintyMulti_safe","UncertaintyMulti_Risk_safe"),
                    labels=c("Uncertainty","Risk"),values = c(wes_palette("Royal1")[2],wes_palette("Royal1")[1]))+
...
DKLDummy%<>%
  select(subject,age,probGamble,DFE1DFD0,OtherChoseRisk,alpha,beta,UncertaintyMulti_risk,UncertaintyMulti_Risk_risk,UncertaintyMulti_Risk_safe,UncertaintyMulti_safe)%>%
  unique()%>%filter(OtherChoseRisk!="NULL")%>%
  rowwise()%>%
  mutate(DKLRisk=case_when((DFE1DFD0==1 & OtherChoseRisk=="1")~KL_divergence_beta(alpha,beta,alpha+UncertaintyMulti_risk,beta),
                           (DFE1DFD0==0 & OtherChoseRisk=="1")~KL_divergence_beta(alpha,beta,alpha+UncertaintyMulti_Risk_risk,beta)),
         DKLSafe=case_when((DFE1DFD0==1 & OtherChoseRisk=="0")~KL_divergence_beta(alpha,beta,alpha,beta+UncertaintyMulti_safe),
                           (DFE1DFD0==0 & OtherChoseRisk=="0")~KL_divergence_beta(alpha,beta,alpha,beta+UncertaintyMulti_Risk_safe))
  )
...
DKLDummy%>%pivot_longer(cols=c(DKLRisk,DKLSafe))%>%
  ...
  stat_summary(aes(group=interaction(DFE1DFD0,subject)),fun.y="mean",geom="point", size=2)+
  #  geom_hline(yintercept = 1,size=1)+
  scale_x_continuous(name="Age")+
  scale_y_continuous(name=expression("KL-Divergence"))+
  scale_color_manual(name="",
                     breaks=c("1","0"),
                     labels=c("Experienced Risks","Described Risks"),values = wes_palette("Royal1"))+
...
# 
ParameterModel3<-brm(mvbind(sigma_sq_riskRisk,
                            sigma_sq_riskUnc,
                            rho,
                            UncertaintyMulti_risk,
                            UncertaintyMulti_Risk_risk,
                            UncertaintyMulti_Risk_safe,
                            UncertaintyMulti_safe)~linearAge+quadraticAge,data = Parameters_wide_poly,cores = 4,iter = 2000,control=list(adapt_delta=0.99999,
                                                                                                                                         max_treedepth = 20)
)
...
brm(UncertaintyMulti_Risk_safe~linearAge+quadraticAge, data=Parameters_wide_poly)
...
summarized_Model=rbind(
  ...
  Parameters_wide_poly%>%select("UncertaintyMulti_risk","age","subject")%>%
    add_column(as.tibble(modelpreds[,,'UncertaintyMultirisk']))%>%mutate(value=UncertaintyMulti_risk,parameter="UncertaintyMulti_risk")%>%select(-UncertaintyMulti_risk),
  
  Parameters_wide_poly%>%select("UncertaintyMulti_Risk_risk","age","subject")%>%
    add_column(as.tibble(modelpreds[,,'UncertaintyMultiRiskrisk']))%>%mutate(value=UncertaintyMulti_Risk_risk,parameter="UncertaintyMulti_Risk_risk")%>%select(-UncertaintyMulti_Risk_risk),
  
  Parameters_wide_poly%>%select("UncertaintyMulti_Risk_safe","age","subject")%>%
    add_column(as.tibble(modelpreds[,,'UncertaintyMultiRisksafe']))%>%mutate(value=UncertaintyMulti_Risk_safe,parameter="UncertaintyMulti_Risk_safe")%>%select(-UncertaintyMulti_Risk_safe),
  
  Parameters_wide_poly%>%select("UncertaintyMulti_safe","age","subject")%>%
    add_column(as.tibble(modelpreds[,,'UncertaintyMultisafe']))%>%mutate(value=UncertaintyMulti_safe,parameter="UncertaintyMulti_safe")%>%select(-UncertaintyMulti_safe)
)
...
```{r}
ParameterModel3 %>%
  spread_draws(b_UncertaintyMultirisk_linearAge,
               b_UncertaintyMultirisk_quadraticAge,
               b_UncertaintyMultiRiskrisk_linearAge,
               b_UncertaintyMultiRiskrisk_quadraticAge)%>%
  pivot_longer(cols = c("b_UncertaintyMultirisk_linearAge",
                        "b_UncertaintyMultirisk_quadraticAge",
                        "b_UncertaintyMultiRiskrisk_linearAge",
                        "b_UncertaintyMultiRiskrisk_quadraticAge"))%>%
  mutate(name2=case_when((name=="b_UncertaintyMultirisk_linearAge")~"A",
                         (name=="b_UncertaintyMultirisk_quadraticAge")~"B",
                         (name=="b_UncertaintyMultiRiskrisk_linearAge")~"C",                                                                                                (name=="b_UncertaintyMultiRiskrisk_quadraticAge")~"D"),
         RiskUnc=case_when((name=="b_UncertaintyMultirisk_linearAge")~"Uncertainty",
                           (name=="b_UncertaintyMultirisk_quadraticAge")~"Uncertainty",
                           (name=="b_UncertaintyMultiRiskrisk_linearAge")~"Risk",                                                                                                (name=="b_UncertaintyMultiRiskrisk_quadraticAge")~"Risk",                                            
         ))%>%
  ggplot(aes(y = name2, x = value,fill=RiskUnc)) +
  geom_vline(aes(xintercept=0),linetype="dotdash")+
  scale_y_discrete(name="Age Effects",breaks=c("A","B","C","D"),labels=c("linear\nexperienced","quadratic\nexperienced","linear\ndescribed","quadratic\ndescribed"))+
...
ParameterModel3 %>%
  spread_draws(b_UncertaintyMultisafe_linearAge,
               b_UncertaintyMultisafe_quadraticAge,
               b_UncertaintyMultiRisksafe_linearAge,
               b_UncertaintyMultiRisksafe_quadraticAge)%>%
  pivot_longer(cols = c("b_UncertaintyMultisafe_linearAge",
                        "b_UncertaintyMultisafe_quadraticAge",
                        "b_UncertaintyMultiRisksafe_linearAge",
                        "b_UncertaintyMultiRisksafe_quadraticAge"))%>%filter(!is.na(value))%>%
  mutate(name2=case_when((name=="b_UncertaintyMultisafe_linearAge")~"A",
                         (name=="b_UncertaintyMultisafe_quadraticAge")~"B",
                         (name=="b_UncertaintyMultiRisksafe_linearAge")~"C",                                                                                                (name=="b_UncertaintyMultiRisksafe_quadraticAge")~"D"),
         RiskUnc=case_when((name=="b_UncertaintyMultisafe_linearAge")~"Uncertainty",
                           (name=="b_UncertaintyMultisafe_quadraticAge")~"Uncertainty",
                           (name=="b_UncertaintyMultiRisksafe_linearAge")~"Risk",                                                                                                (name=="b_UncertaintyMultiRisksafe_quadraticAge")~"Risk",                                            
         ))%>%
  ggplot(aes(y = name2, x = value,fill=RiskUnc)) +
  geom_vline(aes(xintercept=0),linetype="dotdash")+
  scale_y_discrete(name="Age Effects",
                   breaks=c("A","B","C","D"),labels=c("linear\nexperienced","quadratic\nexperienced","linear\ndescribed","quadratic\ndescribed"))+
  scale_x_continuous(name=expression(beta))+
  ggdist::stat_slab()+
  #ggdist::stat_interval(.width = c(.50, .80, .95)) +
  scale_color_brewer(name="HDI",palette = "Greys")+
...
<!-- ```{r} -->
<!-- summarized%>%filter(str_detect(parameter,"UncertaintyMulti"))%>%mutate(UncOrRisk=case_when( -->
<!--   (parameter=="UncertaintyMulti_Risk_risk")~"Risk", -->
<!--   (parameter=="UncertaintyMulti_Risk_safe")~"Risk", -->
<!--   (parameter=="UncertaintyMulti_risk")~"Uncertainty", -->
<!--   (parameter=="UncertaintyMulti_safe")~"Uncertainty") -->
<!-- )->PosteriorTTest -->
...
```{r}
HowManyRecoveries=199
allRecoverey=NULL
for (i in 1:HowManyRecoveries){
  rec=readRDS(paste0("F_ParameterRecovery/backup_05_04_2022/Iter2_",i))
  allRecoverey=rbind(allRecoverey,rec)
}

Parameters=c("kappa","rho","Social_Risk","Social_Unc","weight")
ParametersSim=c("kappa","rho","Social_Risk","Social_Unc","weight")


Corrs=matrix(NA,nrow = length(Parameters),ncol=length(Parameters))%>%
  magrittr::set_colnames(Parameters)%>%
  magrittr::set_rownames(ParametersSim)

for (x in 1:length(Parameters)){
  for (y in 1:length(Parameters)){
    Corrs[x,y]=cor(allRecoverey%>%filter(Parameter2==Parameters[x])%>%.$Recov,
                   allRecoverey%>%filter(Parameter2==Parameters[y])%>%.$Sim)
  }
}
...
allRecoverey%>%group_by(Parameter2)%>%summarize(COR=cor(Sim,Recov))


ParametersLab=c(expression(sigma*" unc"),expression(sigma*" risk"),expression(psi*" unc"),expression(psi*" risk"),expression(lambda))
...
ggplot(data = melted_cormat, aes(x=Var1, y=Var2, fill=value)) + 
  ...
  scale_y_discrete("Fitted Parameter",breaks=Parameters,labels=ParametersLab)+
  scale_x_discrete("Simulation Parameter",breaks=Parameters,labels=ParametersLab)+
  theme_minimal(16)+theme(aspect.ratio = 1)

ggsave(filename = "X_Figures/Recovery2.png")
```
...
tibble(
  ...
  #geom_rect(xmin=0, ymin=0, xmax=5^0.5, ymax=23,fill="azure2",color="white",alpha=0.1)+
  #geom_rect(xmin=5^0.5, ymin=0, xmax=20^0.5, ymax=23,fill="lightcoral",color="white",alpha=0.1)+
  geom_line(alpha=0.5)+
  ...
  # annotate(geom="text", x=2.7, y=0.65, label="ChooseRisk",color="red")+
  #  annotate(geom="text", x=1.8, y=0.65, label="ChooseSafe",color="blue")+
