...
for (agegroup in 1:5){
  ...
  marble_mod=readRDS(paste0("C_ModelFits/model3_age",agegroup,".rds"))
  ...
  param_df=rbind(param_df,rho_dummy,tau_dummy,kappa_dummy)
  
}

joined_marbles=inner_join(MarbleData,param_df,by = "participant")

joined_marbles%>%ggplot(aes(x=age,y=m))+
  geom_point(aes(color=agegroup))+
  stat_smooth(method="lm")+
  facet_wrap(.~ param, scales="free")

for_reg<-joined_marbles%>%pivot_wider(values_from = "m",names_from = "param")
...
logliks_util=list()
logliks_rewsens=list()
logliks_bayes=list()

for (agegroup in 1:4){
  logliks_util[[agegroup]]=readRDS(paste0("C_ModelFits/model1_age",agegroup,".rds"))%>%loo::extract_log_lik()
  logliks_rewsens[[agegroup]]=readRDS(paste0("C_ModelFits/model2_age",agegroup,".rds"))%>%loo::extract_log_lik()
  logliks_bayes[[agegroup]]=readRDS(paste0("C_ModelFits/model3_age",agegroup,".rds"))%>%loo::extract_log_lik()
}

logliks_util_all<-do.call("cbind",logliks_util)
logliks_rewsens_all<-do.call("cbind",logliks_rewsens)
logliks_bayes_all<-do.call("cbind",logliks_bayes)

util_loo<-logliks_util_all%>%loo()
bayes_loo<-logliks_bayes_all%>%loo()
rewsens_loo<-logliks_rewsens_all%>%loo()

loo_compare(bayes_loo,util_loo,rewsens_loo)
```
