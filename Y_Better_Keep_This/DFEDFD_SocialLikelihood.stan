...
parameters {
	...
	real<lower=0> mu_alphaDFD;
	real<lower=0> mu_betaDFD;
	...
	real<lower=0> sigma_alphaDFD;
	real<lower=0> sigma_betaDFD;
...
//	real<lower=0, upper=3> beta_add[N];
	real<lower=0> tau[N];
	real <lower=0> RiskMulti[N];
	real <lower=0> alphaDFD[N];
	real <lower=0> betaDFD[N];
	real <lower=0> UncertaintyMulti[N];
	...
	for (i in 1:N) {// subs
		...
		for (t in 1:Tsubj[i])
  		if(risk1Unc0[i, t]==0){
    		alphaDFD[i,t]= Phi_approx(mu_alphaDFD + sigma_alphaDFD * alpha_alphaDFD[i,t]);
...
model {
  ...
  mu_betaDFD~ normal(1,1);
  mu_alphaDFD~ normal(1,1);
	...
	sigma_alphaDFD~ normal(0,0.2);
	sigma_betaDFD~ normal(0,0.2);
	...
	for (i in 1:N) {
		...
		for (t in 1:Tsubj[i]) {
			...
			// is it a risk trial?
			if(risk1Unc0[i, t]==0){
			  alphaDFD_p[i,t]~normal(0,1.0);
	  	  betaDFD_p[i,t]~normal(0,1.0);
	  	  
			  p_gamble[i,t] ~ beta((alphaDFD[i,t]),(beta_add[i,t])); // can i estimate that?
				U_safe  = pow(safe_payoff[i, t], rho[i]);
				U_risky = p_gamble[i,t] * pow(risky_payoff[i, t], rho[i]);
				
				if (condition[i, t] == 1) {  // safe-safe
					p_gambleU[i,t] ~ beta((alphaDFD[i,t]),(betaDFD[i,t]*UncertaintyMulti[i])); 
					U_risky = p_gambleU[i,t] * pow(risky_payoff[i, t], rho[i]);
				}
				 if (condition[i, t] == 3) {  // risky-risky
				 		p_gambleU[i,t] ~ beta((alphaDFD[i,t]*UncertaintyMulti[i]),(betaDFD[i,t])); 
						U_risky = p_gambleU[i,t] * pow(risky_payoff[i, t], rho[i]);
				 }
