...
#i=1;
#k=5;
PATH_LOG_E="./logsE/$(date '+%Y%m%d_%H%M%S')/slurm-%j.err"
PATH_LOG_O="./logsY/$(date '+%Y%m%d_%H%M%S')/slurm-%j.out"
...
do
	...
	#do
      ...
      echo "#SBATCH --job-name LBFGS-Marb${i}_${j}"     >> job.slurm
      echo "#SBATCH --partition long"                   >> job.slurm
      echo "#SBATCH --mem 5GB"                          >> job.slurm
      echo "#SBATCH --cpus 1"                           >> job.slurm
      echo "#SBATCH --time 2:30:0"                     >> job.slurm
      echo "#SBATCH --workdir ."                        >> job.slurm
      echo "#SBATCH --error ${PATH_LOG_E}"              >> job.slurm
      echo "#SBATCH --output ${PATH_LOG_O}"             >> job.slurm
			echo "module load R/4; Rscript ParameterRecovery.R $i $j"	>>  job.slurm
    sbatch job.slurm
    rm -f job.slurm
		done
	done
