...
parameters {
	...
	// Social Parameters
	real<lower=0> UncertaintyMulti[N,T];
  			...
  			if(risk1Unc0[i, t]==0){
  			  ...
  			  U_risky = pow(risky_payoff[i, t], rho[i]);
  				...
  				if (condition[i, t] == 1) {  // safe-safe
  				  UncertaintyMulti[i,t] ~ normal(1,0.5);
    				newBeta=newBeta+UncertaintyMulti[i,t];// beta to the power of Social Info
  				}if (condition[i, t] == 3) {  // risky-risky
  				  UncertaintyMulti[i,t] ~ normal(1,0.5);
    				newAlpha=newAlpha+UncertaintyMulti[i,t];//alpha to the power of Social Info				 }
  				  }//end Social
  			 }//end  	Risk
