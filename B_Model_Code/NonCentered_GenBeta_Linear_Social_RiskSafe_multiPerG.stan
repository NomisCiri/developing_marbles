...
parameters {
  ...
  real<lower=0, upper=2> mu_alpha_add_risk;
  real<lower=0, upper=2> mu_alpha_add_safe;
  ...
  // Social Parameters
  real<lower=0> UncertaintyMulti_safe[N,NGambles];
  real<lower=0> UncertaintyMulti_Risk_safe[N,NGambles];
  
  real<lower=0> UncertaintyMulti_risk[N,NGambles];
  real<lower=0> UncertaintyMulti_Risk_risk[N,NGambles];
  //
...
model {
  ...
  for (i in 1:N) {
    ...
    UncertaintyMulti_risk[i,gambles] ~ normal(2, 1);
    UncertaintyMulti_Risk_risk[i,gambles] ~ normal(1, 1);
    UncertaintyMulti_safe[i,gambles] ~ normal(2, 1);
    UncertaintyMulti_Risk_safe[i,gambles] ~ normal(1, 1);
    }
    //check trials.
    for (t in 1:Tsubj[i]) {
      ...
      if(risk1Unc0[i, t]==0){
        ...
        newBeta=beta_shape[i,p_gamble[i,t]]*UncertaintyMulti_Risk_safe[i,p_gamble[i,t]];// beta to the power of Social Info
        }if (condition[i, t] == 3) {  // risky-risky
        newAlpha=alpha_shape[i,p_gamble[i,t]]*UncertaintyMulti_Risk_risk[i,p_gamble[i,t]];//alpha to the power of Social Info				 }
        }//end Social
      }//end  	Risk
      ...
      if (condition[i, t] == 1) {  // safe-safe
      newBeta=estimateBeta*UncertaintyMulti_safe[i,p_gamble[i,t]];// beta to the power of Social Info
      }if (condition[i, t] == 3) {  // risky-risky
      newAlpha=estimateAlpha*UncertaintyMulti_risk[i,p_gamble[i,t]];//alpha to the power of Social Info				 }
      }
