...
model {
  ...
  for (i in 1:N) {
    ...
    alpha_add[i] ~ normal(0,1.0);
    //centred parametization works for these parameters. 
    UncertaintyMulti[i] ~ normal(1, 0.5);
    ...
    for (t in 1:Tsubj[i]) {
      ...
      if(risk1Unc0[i, t]==0){
        ...
        U_risky  = pow(risky_payoff[i, t], rho[i]+UncertaintyMulti[i]);
...
generated quantities {
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      ...
      log_lik[i,t] = bernoulli_lpmf(choice[i,t]|1-generalized_beta_cdf(U_safe, newAlpha,newBeta,0.00001,U_risky));
