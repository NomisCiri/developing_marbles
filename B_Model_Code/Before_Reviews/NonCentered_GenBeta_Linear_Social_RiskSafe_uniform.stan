...
real generalized_beta_cdf(real x, real alpha, real beta, real end){
  real z;
  real prob;
  z = inv_logit((x-end));//rescale the input value... calculates the "integral up until the safe option"
...
model {
  ...
  for (i in 1:N) {
    ...
    //tau[i]~normal(1,0.1);
    //centred parametization works better for these parameters. 
    UncertaintyMulti_risk[i] ~ normal(2,10);
    UncertaintyMulti_Risk_risk[i] ~ normal(2,10);
    UncertaintyMulti_safe[i] ~ normal(2,10);
    UncertaintyMulti_Risk_safe[i] ~ normal(2,10);
    ...
    //alpha_shape[i,gambles]~gamma(2,2);//lets see.
    kappa[i,gambles]~gamma(3,0.5);//more flexible here. Make hyperprior for diffbles.
    ...
    for (t in 1:Tsubj[i]) {
      ...
      if(risk1Unc0[i, t]==0){
        ...
        newBeta=newBeta*UncertaintyMulti_Risk_safe[i];// beta to the power of Social Info
        }if (condition[i, t] == 3) {  // risky-risky
        newAlpha=newAlpha*UncertaintyMulti_Risk_risk[i];//alpha to the power of Social Info				 }
        }//end Social
      }//end  	Risk
      ...
      //print(choice[i, t])
      choice[i, t] ~ bernoulli(1-generalized_beta_cdf(U_safe, newAlpha,newBeta,U_risky,tau[i]));
    } //endTrail
  }//endSubs
}//endModel
...
generated quantities {
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      ...
      log_lik[i,t]= bernoulli_lpmf(choice[i,t]|1-generalized_beta_cdf(U_safe, newAlpha,newBeta,U_risky,tau[i]));
      y_pred[i, t] = bernoulli_rng(1-generalized_beta_cdf(U_safe, newAlpha,newBeta,U_risky,tau[i]));// each choice is predicted as a bernoulli distributed random variable.		} //endTrail
    } //endSub
  }//end PostPred
}
