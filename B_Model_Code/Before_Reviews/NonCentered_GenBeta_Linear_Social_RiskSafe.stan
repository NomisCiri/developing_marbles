...
data {
  ...
  int<lower=1> unc[2];//uncertainty condition to loop over
  ...
  int<lower=0, upper=1>risk1Unc2[N,T]; 
...
parameters {
  ...
  //
  real<lower=0> kappa[N,NGambles,unc];
...
transformed parameters {
  ...
  real<lower=0>alpha_shape[N,NGambles,2];
  real<lower=0>beta_shape[N,NGambles,2];
  ...
  for(gambles in 1:NGambles){
    for(u in unc){
      alpha_shape[i,gambles,u]=gambleList[gambles]*(kappa[i,gambles,u]);
      beta_shape[i,gambles,u]=(1-gambleList[gambles])*(kappa[i,gambles,u]);
    }//end Gambleloop
  }//endsubs
...
model {
  ...
  kappa~gamma(2,0.5);
  rho_p ~ normal(0, 1.0);
  alpha_add_p ~ normal(0,1.0);
  psi_risk ~ normal(1,5);
  psi_Risk_risk ~ normal(1,5);
  psi_safe ~ normal(1,5);
  psi_Risk_safe ~ normal(1,5);
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      ...
      if (condition[i, t] == 1) {  // safe-safe
      newBeta=beta_shape+psi_safe[i];// beta to the power of Social Info
      }if (condition[i, t] == 3) {  // risky-risky
      newAlpha=alpha_shape+psi_risk[i];//alpha to the power of Social Info
      }
      // a bernoulli distributed random variable.
      }//end Uncertain
      
      ev_diff=
      choice[i, t] ~ bernoulli_logit(ev_diff);
    } //endTrail
  }//endSubs
}//endModel
