...
//	real<lower=0, upper=3> beta_add[N];
	...
	for (i in 1:N) {// subs
		...
		for(gambles in 1:NGambles){
		  alpha_shape[i,gambles]=gambleList[gambles]*(kappa[i]);
		  beta_shape[i,gambles]=(1-gambleList[gambles])*(kappa[i]);
		}//end Gambleloop
	}//endsubs
...
model {
	...
	for (i in 1:N) {
      ...
      //centred parametization works for these parameters. 
  		UncertaintyMulti_risk[i] ~ normal(1,0.4);
  		UncertaintyMulti_Risk_risk[i] ~ normal(1,0.4);
  		UncertaintyMulti_safe[i] ~ normal(1,0.4);
  		UncertaintyMulti_Risk_safe[i] ~ normal(1,0.4);
  		...
  		for (gambles in 1:NGambles){//gambleUncertainty Loop
  		  //alpha_shape[i,gambles]~gamma(2,2);//lets see.
        kappa[i]~gamma(1,0.1);//more flexible here. Make hyperprior for diffbles.
        gambleList[gambles]~beta_proportion(gambleList[gambles],kappa[i]);
      }
      //check trials.
  		for (t in 1:Tsubj[i]) {
  			...
  			if(risk1Unc0[i, t]==0){
  			  ...
  			  U_risky = pow(risky_payoff[i, t], rho[i]);
  				...
  				if (condition[i, t] == 1) {  // safe-safe
    				newBeta=newBeta+UncertaintyMulti_Risk_safe[i];// beta + Social Info
  				}if (condition[i, t] == 3) {  // risky-risky
    				newAlpha=newAlpha+UncertaintyMulti_Risk_risk[i];//alpha + Social Info				 }
  				  }//end Social
  			 }//end  	Risk
    		...
    		//  print(choice[i, t])
  //   		  print("risky_payoff")
  // 			  print(risky_payoff[i, t])
  //         print(rho[i])
        //print(1-generalized_beta_cdf(U_safe, newAlpha,newBeta,U_risky))
