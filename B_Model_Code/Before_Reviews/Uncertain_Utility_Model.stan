...
transformed parameters {
  ...
  // updating Model
  alpha_add[i]= Phi_approx(mu_alpha_add + sigma_alpha_add * alpha_add_p[i]);
  // from Wikipedia: α = μ*kappa, β = (1 − μ)*kappa
  //get back the shape parameters. 
  for(gambles in 1:NGambles){
    alpha_shape[i,gambles]=gambleList[gambles]*(kappa[i,gambles])+1;
    beta_shape[i,gambles]=(1-gambleList[gambles])*(kappa[i,gambles])+1;
  }//end Gambleloop
  }//endsubs
...
model {
  ...
  for (i in 1:N) {
    //print("log density after sigmarho",[i]," before kappa =", target());
   ...
   // print("log density after sigmarho before sigmaalphaadd =", target());
    UncertaintyMulti_risk[i] ~ normal(1,1);
    UncertaintyMulti_Risk_risk[i] ~ normal(1,1);
    UncertaintyMulti_safe[i] ~ normal(1,1);
    ...
    kappa[i,gambles]~normal(9,2);//more flexible here. Make hyperprior for diffbles.
...
generated quantities {
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      ...
      log_lik[i]+= bernoulli_logit_lpmf(choice[i,t]|1-generalized_beta_cdf(U_safe, newAlpha,newBeta,0.00001,U_risky));
