...
parameters {
	...
	// Social Parameters
	real mu_ocu_safe_Risk;
	real mu_ocu_safe_Uncertainty;
	
	real mu_ocu_risk_Risk;
	real mu_ocu_risk_Uncertainty;
	//roup Sigmas
	real<lower=0>sigma_rho;
	real<lower=0>sigma_tau;
	
	real<lower=0>sigma_ocu_safe_Risk;
	real<lower=0>sigma_ocu_safe_Uncertainty;
	
	real<lower=0>sigma_ocu_risk_Risk;
	real<lower=0>sigma_ocu_risk_Uncertainty;
	...
	real ocu_safe_Risk_p[N];
	real ocu_safe_Uncertainty_p[N];
	real ocu_risk_Risk_p[N];
	real ocu_risk_Uncertainty_p[N];
...
transformed parameters {
	...
	real<lower=0, upper=3> alpha_add[N];
//	real<lower=0, upper=3> beta_add[N];
	
	real<lower=0> tau[N];
	
	real  ocu_risk_Risk[N];
	real  ocu_risk_Uncertainty[N];
	
	real  ocu_safe_Risk[N];
	real  ocu_safe_Uncertainty[N];
	...
	for (i in 1:N) {// subs
		...
		ocu_safe_Risk[i] = mu_ocu_safe_Risk + sigma_ocu_safe_Risk * ocu_safe_Risk_p[i];
		ocu_safe_Uncertainty[i] = mu_ocu_safe_Uncertainty + sigma_ocu_safe_Uncertainty * ocu_safe_Uncertainty_p[i];
		
		ocu_risk_Risk[i] = mu_ocu_risk_Risk + sigma_ocu_risk_Risk * ocu_risk_Risk_p[i];
		ocu_risk_Uncertainty[i] = mu_ocu_risk_Uncertainty + sigma_ocu_risk_Uncertainty * ocu_risk_Uncertainty_p[i];
	  
		// updating Model
		alpha_add[i]= Phi_approx(mu_alpha_add + sigma_alpha_add * alpha_add_p[i])*3;
...
model {
	...
	mu_ocu_safe_Risk  ~ normal(0,1);
	mu_ocu_safe_Uncertainty  ~ normal(0,1);
	
	mu_ocu_risk_Risk  ~ normal(0,1);
	mu_ocu_risk_Uncertainty  ~ normal(0,1);
	...
	sigma_ocu_safe_Risk ~ cauchy(0, 1);
	sigma_ocu_safe_Uncertainty ~ cauchy(0, 1);
	
	sigma_ocu_risk_Risk ~ cauchy(0, 1);
	sigma_ocu_risk_Uncertainty ~ cauchy(0, 1);
	...
	for (i in 1:N) {
		...
		ocu_safe_Risk_p[i] ~ normal(0, 1.0);
		ocu_safe_Uncertainty_p[i] ~ normal(0, 1.0);
		
		ocu_risk_Risk_p[i] ~ normal(0, 1.0);
		ocu_risk_Uncertainty_p[i] ~ normal(0, 1.0);
		...
		for (t in 1:Tsubj[i]) {
			...
			if(risk1Unc0[i, t]==0){
				...
				if (condition[i, t] == 1) {  // safe-safe
					U_safe = U_safe + ocu_safe_Risk[i];        
				}
				 if (condition[i, t] == 3) {  // risky-risky
						U_risky = U_risky + ocu_risk_Risk[i];
					}
		 ...
		 if(risk1Unc0[i, t]==1){// is it an uncertain trial?
			...
			// 	}
				...
				p_gamble_est[i,t] ~ beta(pow(Sucess[i,t],alpha_add[i]),pow(Fail[i,t],alpha_add[i])); // we know that the winning probability is beta distributed with the obtained shape parameters. 
				//beta_rng(alpha_total,beta_total)
				U_safe  = pow(safe_payoff[i, t], rho[i]);
				U_risky = p_gamble_est[i,t] * pow(risky_payoff[i, t], rho[i]);
				
				if (condition[i, t] == 1) {  // safe-safe
					U_safe = U_safe + ocu_safe_Uncertainty[i];        
				}
				 if (condition[i, t] == 3) {  // risky-risky
						U_risky = U_risky + ocu_risk_Uncertainty[i];
					}
					choice[i, t] ~ bernoulli_logit((tau[i])* (U_risky - U_safe));
				}//end Uncertain
