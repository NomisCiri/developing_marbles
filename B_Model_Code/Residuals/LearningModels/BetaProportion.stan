...
parameters {
  ...
  real<lower=0> mu_Prec_Beta[NGambles];// TODO: Set Boundary between 0 and 0.25
  ...
  real<lower=0>sigma_Prec_Beta[NGambles];// TODO: Set Boundary between 0 and 0.25
  ...
  real alpha_add_p[N];
  real Prec_Beta_p[N,NGambles];// TODO: Set Boundary between 0 and 0.25

  
  real<lower=0, upper=1>p_gambleU[N, T];
  real<lower=0, upper=1>p_gambleR[N,NGambles];
  //real<lower=0,upper=50> HyperpriorUnc[NGambles];
}
...
transformed parameters {
  ...
  real<lower=0, upper=200> Prec_Beta[N,NGambles];
  real<lower=0.01> alpha[N,NGambles];
  real<lower=0.01> beta[N,NGambles];
  ...
  for (i in 1:N) {// subs
    ...
    for (t in 1:NGambles){
      Prec_Beta[i,t] = Phi_approx(mu_Prec_Beta[t] + sigma_Prec_Beta[t] * Prec_Beta_p[i,t])*200;//noncentered gamble precicion doing that +0.1 to get the right initial values.
		  alpha[i,t] = ((1 - gambleList[t]) / (1/Prec_Beta[i,t]) - 1 / gambleList[t]) * pow(gambleList[t],2);
      beta[i,t] = alpha[i,t] * (1 / gambleList[t] - 1);
		}
  }//endsubs
...
model {
  ...
  for (gambles in 1:NGambles){
    mu_Prec_Beta[gambles] ~ normal(0,0.2);
    sigma_Prec_Beta[gambles]~ cauchy(0, 1);
  }
  ...
  for (i in 1:N) {
    ...
    for (gambles in 1:NGambles){//gambleUncertainty Loop
      //HyperpriorUnc[gambles]~normal(20,10);//lets see.
      Prec_Beta_p[i,gambles]~normal(0,1);//more flexible here. Make hyperprior for diffbles.
      gambleList[gambles]~ beta_proportion(gambleList[gambles],Prec_Beta[i,gambles]);
    }
    //check trials.
    for (t in 1:Tsubj[i]) {
      real U_safe;
      ...
      if(risk1Unc0[i, t]==0){
        ...
        if (condition[i, t] == 1) {  // safe-safe
          p_gambleR[i,p_gamble[i,t]]~beta(alpha[i,p_gamble[i,t]],beta[i,p_gamble[i,t]]*UncertaintyMulti_Risk[i]);
          U_risky = p_gambleR[i,p_gamble[i,t]] * pow(risky_payoff[i, t], rho[i]);
        }
        if (condition[i, t] == 3) {  // risky-risky
          p_gambleR[i,p_gamble[i,t]]~beta(alpha[i,p_gamble[i,t]]*UncertaintyMulti_Risk[i],beta[i,p_gamble[i,t]]);
          U_risky = p_gambleR[i,p_gamble[i,t]] * pow(risky_payoff[i, t], rho[i]);
        }
