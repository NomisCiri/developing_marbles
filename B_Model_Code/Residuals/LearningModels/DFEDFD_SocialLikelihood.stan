...
parameters {
	...
	real<lower=0,upper=0.25>Unc_Beta[N,NGambles];// TODO: Set Boundary between 0 and 0.25
...
//print(Unc_Beta[i,t]);
		  ...
		  alpha[i,t] = ((1 - gambleList[t]) / Unc_Beta[i,t] - 1 / gambleList[t]) * pow(gambleList[t],2);
...
model {
  ...
  mu_UncertaintyMulti_Risk  ~ normal(1,0.5);
	...
	for (i in 1:N) {
      ...
      Unc_Beta[i,gambles]~exponential(HyperpriorUnc[gambles]);//more flexible here. Make hyperprior for diffbles.
