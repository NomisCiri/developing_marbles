...
parameters {
  ...
  // Social Parameters
  real<lower=0, upper=1> TremblingHand[N];
  //
...
transformed parameters {
  ...
  for(gambles in 1:NGambles){
    alpha_shape[i,gambles]=gambleList[gambles]*(kappa[i,gambles]-2);
    beta_shape[i,gambles]=(1-gambleList[gambles])*(kappa[i,gambles]-2);
  }//end Gambleloop
  }//endsubs
...
model {
  ...
  for (i in 1:N) {
    ...
    TremblingHand[i] ~ normal(0.1, 0.01);
    ...
    kappa[i,gambles]~gamma(7,1);//more flexible here. Make hyperprior for diffbles.
    ...
    for (t in 1:Tsubj[i]) {
      ...
      if(risk1Unc0[i, t]==0){
        ...
        U_risky = pow(risky_payoff[i, t], rho[i]);
        choice[i, t] ~ bernoulli((1-TremblingHand[i])*(1-generalized_beta_cdf(U_safe, newAlpha,newBeta,0.00001,U_risky))+TremblingHand[i]/2);// each choice is 
        } else if (condition[i, t] == 3) {  // risky-risky
        U_risky = pow(risky_payoff[i, t], rho[i]);
        choice[i, t] ~ bernoulli((1-TremblingHand[i])*(1-generalized_beta_cdf(U_safe, newAlpha,newBeta,0.00001,U_risky))+TremblingHand[i]/2);// each choice is 
        } else {
            choice[i, t] ~ bernoulli((1-generalized_beta_cdf(U_safe, newAlpha,newBeta,0.00001,U_risky)));// each choice is 
        }
      }//end  	Risk
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      ...
      if(risk1Unc0[i, t]==0){
        ...
        if (condition[i, t] == 1) {  // safe-safe
        U_risky = pow(risky_payoff[i, t], rho[i]);
          log_lik[i]+= bernoulli_lpmf(choice[i,t]|(1-TremblingHand[i])*(1-generalized_beta_cdf(U_safe, newAlpha,newBeta,0.00001,U_risky))+TremblingHand[i]/2);
         y_pred[i, t] = bernoulli_rng((1-TremblingHand[i])*(1-generalized_beta_cdf(U_safe, newAlpha,newBeta,0.00001,U_risky))+TremblingHand[i]/2);// ea
        ...
        y_pred[i, t] = bernoulli_rng((1-TremblingHand[i])*(1-generalized_beta_cdf(U_safe, newAlpha,newBeta,0.00001,U_risky))+TremblingHand[i]/2);// ea
          ...
          y_pred[i, t] = bernoulli_rng((1-generalized_beta_cdf(U_safe, newAlpha,newBeta,0.00001,U_risky)));//
        }
      }//end Risk
      if(risk1Unc0[i, t]==1){// is it an uncertain trial?
      real socialAlphaUnc;
      ...
      U_risky = pow(risky_payoff[i, t], rho[i]);
      log_lik[i]+= bernoulli_lpmf(choice[i,t]|(1-TremblingHand[i])*(1-generalized_beta_cdf(U_safe, newAlpha,newBeta,0.00001,U_risky))+TremblingHand[i]/2);
      y_pred[i, t] = bernoulli_rng((1-TremblingHand[i])*(1-generalized_beta_cdf(U_safe, newAlpha,newBeta,0.00001,U_risky))+TremblingHand[i]/2);// each choice is 
      }else if (condition[i, t] == 3) {  // risky-risky
      U_risky = pow(risky_payoff[i, t], rho[i]);
      log_lik[i]+= bernoulli_lpmf(choice[i,t]|(1-TremblingHand[i])*(1-generalized_beta_cdf(U_safe, newAlpha,newBeta,0.00001,U_risky))+TremblingHand[i]/2);
      y_pred[i, t] = bernoulli_rng((1-TremblingHand[i])*(1-generalized_beta_cdf(U_safe, newAlpha,newBeta,0.00001,U_risky))+TremblingHand[i]/2);// each choice is 
