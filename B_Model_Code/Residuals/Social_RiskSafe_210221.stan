functions {  // declare and define user-defined function before calling it
  ...
  real generalized_beta_cdf(real x, real alpha, real beta, real end){
    real z;
    real prob;
  	...
  	//interesting, seems to make a huge difference how many digits i allow for this boundary condition.
  	//everything less then 5 digits after the comma breaks the sampler.
  	if (prob>0.99999){
  	  prob=0.99999;
  	}if(prob<0.00001){
  	  prob=0.00001;
...
parameters {
	...
	real alpha_add_p[N];
	// Social Parameters
	real<lower=0> UncertaintyMulti_safe_mu;
	real<lower=0> UncertaintyMulti_Risk_safe_mu;

	real<lower=0> UncertaintyMulti_risk_mu;
	real<lower=0> UncertaintyMulti_Risk_risk_mu;
	
	real<lower=0> UncertaintyMulti_safe_sigma;
	real<lower=0> UncertaintyMulti_Risk_safe_sigma;

	real<lower=0> UncertaintyMulti_risk_sigma;
	real<lower=0> UncertaintyMulti_Risk_risk_sigma;
	
	real UncertaintyMulti_safe_p[N];
	real UncertaintyMulti_Risk_safe_p[N];

	real  UncertaintyMulti_risk_p[N];
	real  UncertaintyMulti_Risk_risk_p[N];
	//
...
transformed parameters {
	...
	real<lower=0, upper=2> UncertaintyMulti_safe[N];
	real<lower=0,upper=2> UncertaintyMulti_Risk_safe[N];

	real<lower=0,upper=2> UncertaintyMulti_risk[N];
	real<lower=0,upper=2> UncertaintyMulti_Risk_risk[N];
//	real<lower=0, upper=3> beta_add[N];
	...
	for (i in 1:N) {// subs
		...
		UncertaintyMulti_safe[i]=Phi_approx(UncertaintyMulti_safe_mu+UncertaintyMulti_safe_sigma+UncertaintyMulti_safe_p[i])*2;
		UncertaintyMulti_risk[i]=Phi_approx(UncertaintyMulti_risk_mu+UncertaintyMulti_risk_sigma+UncertaintyMulti_risk_p[i])*2;
		UncertaintyMulti_Risk_safe[i]=Phi_approx(UncertaintyMulti_Risk_safe_mu+UncertaintyMulti_Risk_safe_sigma+UncertaintyMulti_Risk_safe_p[i])*2;
		UncertaintyMulti_Risk_risk[i]=Phi_approx(UncertaintyMulti_Risk_risk_mu+UncertaintyMulti_Risk_risk_sigma+UncertaintyMulti_Risk_risk_p[i])*2;
...
model {
	...
	UncertaintyMulti_safe_mu~normal(1,1);
	UncertaintyMulti_Risk_safe_mu~normal(0.5,1);

	UncertaintyMulti_risk_mu~normal(0.5,1);
	 UncertaintyMulti_Risk_risk_mu~normal(1,1);
	
	UncertaintyMulti_safe_sigma~ cauchy(1, 1);
	UncertaintyMulti_Risk_safe_sigma~ cauchy(1, 1);

	UncertaintyMulti_risk_sigma~ cauchy(1, 1);
	UncertaintyMulti_Risk_risk_sigma~ cauchy(1, 1);
	...
	for (i in 1:N) {
      ...
      //centred parametization works for these parameters. 
  		UncertaintyMulti_risk_p[i] ~ normal(0,1);
  		UncertaintyMulti_Risk_risk_p[i] ~ normal(0,1);
  		UncertaintyMulti_safe_p[i] ~ normal(0,1);
  		UncertaintyMulti_Risk_safe_p[i] ~ normal(0,1);
  		  ...
  		  // internal variables that i can throw away afterwards 
  			...
  			if(risk1Unc0[i, t]==0){
  			  ...
  			  U_risky = pow(risky_payoff[i, t], rho[i]);
  				...
  				if (condition[i, t] == 1) {  // safe-safe
    				newBeta=+UncertaintyMulti_Risk_safe[i];// beta to the power of Social Info
  				}if (condition[i, t] == 3) {  // risky-risky
    				newAlpha=+UncertaintyMulti_Risk_risk[i];//alpha to the power of Social Info				 }
  				  }//end Social
  			 }//end  	Risk
    		 if(risk1Unc0[i, t]==1){// is it an uncertain trial?
    		  ...
    		  //print(p_gamble_est[i,t])
    			...
    			if (condition[i, t] == 1) {  // safe-safe
      			newBeta=+UncertaintyMulti_safe[i];// beta to the power of Social Info
    			}if (condition[i, t] == 3) {  // risky-risky
      			newAlpha=+UncertaintyMulti_risk[i];//alpha to the power of Social Info
    			}
