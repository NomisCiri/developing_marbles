...
model {
  ...
  for (i in 1:N) {
    ...
    UncertaintyMulti_risk[i] ~ normal(0.5,3);
    UncertaintyMulti_Risk_risk[i] ~ normal(1,3);
    UncertaintyMulti_safe[i] ~ normal(1,3);
    UncertaintyMulti_Risk_safe[i] ~ normal(0.5,3);
    ...
    //alpha_shape[i,gambles]~gamma(2,2);//lets see.
    kappa[i,gambles]~gamma(5,0.5);//more flexible here. Make hyperprior for diffbles.
