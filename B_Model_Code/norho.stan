  		...
  		for (t in 1:Tsubj[i]) {
  		  // internal variables that i can throw away afterwards 
  			...
  			if(risk1Unc0[i, t]==0){
  			  ...
  			  U_safe  = safe_payoff[i, t];
  			  U_risky = risky_payoff[i, t];
  				//SocialInfoBlock: Overwrite the shape parameters
  				if (condition[i, t] == 1) {  // safe-safe
    				newBeta=beta_shape[i,p_gamble[i,t]]*UncertaintyMulti[i];// beta to the power of Social Info
  				}if (condition[i, t] == 3) {  // risky-risky
    				newAlpha=alpha_shape[i,p_gamble[i,t]]*UncertaintyMulti[i];//alpha to the power of Social Info				 }
  				  }//end Social
  			 }//end  	Risk
    		 if(risk1Unc0[i, t]==1){// is it an uncertain trial?
    		  estimateAlpha=pow(Sucess[i,t],alpha_add[i]);
    		  estimateBeta=pow(Fail[i,t],alpha_add[i]);
    			...
    			if (condition[i, t] == 1) {  // safe-safe
    				newBeta=estimateBeta*UncertaintyMulti[i];// beta to the power of Social Info
    			}if (condition[i, t] == 3) {  // risky-risky
    				newAlpha=estimateAlpha*UncertaintyMulti[i];//alpha to the power of Social Info				 }
    			}
