...
get trial wise choice probabilites for all models
...
for (i in models){
  ...
  for (agegroup in 1:5){
    ...
    MarbleDataD <- MarbleData %>%group_by(participant)%>%
      mutate(max_t=n())%>%
      ungroup()%>%
      filter(max_t==144)%>%
      filter(Agegroup == Agegroups[agegroup])
      subjList <- unique(MarbleDataD$participant) ######
    
    marble_mod=readRDS(paste0("C_ModelFits/model_est_",i,"_age",agegroup,".rds"))
    
    pps<-marble_mod%>%tidybayes::spread_draws(y_pred[ppt,trial])
    
    pps_g<-pps%>%dplyr::group_by(ppt,trial)%>%
      dplyr::summarise(pp=mean(y_pred))%>%
      mutate(participant=subjList[ppt])%>%ungroup()%>%select(-ppt)
    
    pp_df=rbind(pp_df,pps_g)
  }
  pp_container[[cnt]]=inner_join(MarbleData,pp_df,by = c("participant"="participant","trial"="trial"))
  cnt=cnt+1
}

saveRDS(pp_container,file = "A_RawData/pp_list.rds")
...
joined_marbles_pp%>%
  mutate(OtherChoseRisk2=case_when(
    (OtherChoseRisk=="NULL")~"0",
    (OtherChoseRisk=="0")~"-1",
    (OtherChoseRisk=="1")~"1"
  ))%>%ggplot(aes(y=ChooseRisk,x=age,color=DFE1DFD0))+
  stat_summary(mapping=aes(y=ChooseRisk,group=participant),geom="point",fun="mean",shape=23,alpha=0.5)+
  stat_summary(mapping=aes(y=ChooseRisk),geom="pointrange",fun.data="mean_cl_boot",position=position_dodge(0.9),size=1,shape=23)+
  stat_summary(mapping=aes(y=pp),geom="pointrange",fun.data="mean_cl_boot",position=position_dodge(0.9),size=1,shape=23,color="magenta",alpha=0.2)+
  stat_smooth(mapping=aes(x=as.numeric(age)), method="lm",color="black")+
  #models
  #stat_summary(mapping=aes(y=pp,group=participant),geom="point",fun="mean",shape=23,alpha=0.5)+
  #stat_smooth(mapping=aes(x=as.numeric(age)), method="lm",color="black")+
  guides(color=F)+
...
joined_marbles_pp%>%mutate(EV=probGamble*valueGamble,
                           OtherChoseRisk2=case_when(
                             (OtherChoseRisk=="NULL")~"0",
                             (OtherChoseRisk=="0")~"1",
                             (OtherChoseRisk=="1")~"2"
                           ))%>%
  brms::brm(.,formula = ChooseRisk~EV+OtherChoseRisk2*DFE1DFD0*poly(age,2)+(1|participant),cores = 4,family = "Bernoulli")->model
...
#KL-divergence
Here i show how social impact still changes with age despite the lack of age trend in the parameters that govern social informaiton use
...
```{r}
library(brms)
Paramters_wide<-joined_marbles_pp%>%
  pivot_wider(names_from = "parameter",values_from = "posteriorMean",id_cols = c("subject","Agegroup","age"))
