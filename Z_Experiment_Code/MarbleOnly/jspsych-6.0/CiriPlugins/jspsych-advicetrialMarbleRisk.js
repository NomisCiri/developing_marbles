...
jsPsych.plugins["advicetrialMarbleRisk"] = (function() {

	var plugin = {};

	jsPsych.pluginAPI.registerPreload('advicetrialMarbleRisk', 'stimulus', 'image');

	plugin.info = {
		name: 'advicetrialMarbleRisk',
		...
		parameters: {
			...
			valueGamble:{
				type: jsPsych.plugins.parameterType.STRING,
				pretty_name: 'Show Many or few marbles? Default is few (2).',
				default:1
			},
			stimulus1: {
				...
				description: 'The safe stimulus to be displayed'
			},
			stimulus2: {
				type: jsPsych.plugins.parameterType.IMAGE,
				pretty_name: 'stimulus2',
				default: 'img/orange.png',
				description: 'The risky stimulus to be displayed'
			},
			stimulus4: {
				type: jsPsych.plugins.parameterType.IMAGE,
	...
	plugin.trial = function(display_element, trial) {
		...
		var demoChoseRiskArray=new Array;
		...
		/*I Built a matrix which consists of 10 x 5 entries in the first cell of each row the condition
		id is represented. Following are 4 zeros. A zero means that this advice has not been shown before.
		if it has not been shown before, i pick the correponding value in the demochose risk array
		if it was shown before, i go on.*/
		...
		if (!trial.lookUpLastTrial) {
			...
			for (i = 0; i < AllSubsGamble.length; i++){
				if ((AllSubsGamble[i].ppn == demo) && (AllSubsGamble[i].probAv == probWin) && (AllSubsGamble[i].valueGamble == trial.valueGamble)) // check which ID in the demonstrator matches the current trial ID
				{
					demoChoseRisk=Boolean((AllSubsGamble[i].choice));
				}//end check proximity...			
			}//end trialloop	
		} else {//if the youare in the choice section, look uop the last trail and just take these specifications.
			var oldOrder = jsPsych.data.get().last(1).values()[0].old_order;
			var newOrder = jsPsych.data.get().last(1).values()[0].new_order;
			var gambleKey = null;
			demoChoseRisk=Boolean(jsPsych.data.get().last(1).values()[0].OtherChoseRisk);
