...
* risk attitudes. 
* This script deals with the unnested data from Blankenstein.
* 
*
**/
jsPsych.plugins["riskProximity_MarbleRisky"] = (function() {

	var plugin = {};
	plugin.info = {
		name: "riskProximity_MarbleRisky",
		parameters: {
			stimulus_duration: {
	...
	plugin.trial = function(display_element, trial) {
		...
		//var AllSubsGamble = JSON.parse(AllSubsGamble);//Here we have the demonstratorData.	

		display_element.innerHTML = "PROXIMITY IS BEEING CALCULATED";

		var TheirDecisions = Array() //thats a container for decisions of each subject
		var subPercent = Array(0) //thats a container for teh percentage of risky choice for each subject.
		var sub = Array(0) // this one increases parralel to subpercent and stores 
		...
		var k = 0; //Subcount
		var found=false;
		//Here you calculate the percentage of risky choice of all subjects. cant believe this works but it does.
		for (i = 0; i < AllSubsGamble.length; i++) {
			if (i+2 < AllSubsGamble.length) { //better than crashing out of the loop is to end it like that
				if (AllSubsGamble[i].ppn == AllSubsGamble[i + 1].ppn) { //check if ppn is the same as the next one
					TheirDecisions[j] = (AllSubsGamble[i].choice) //yes? concatenate to their decisions.
					j++;
				} else {//end concatanete
					j = 0;//reset 
					subPercent[k] = math.mean(TheirDecisions); //No? probably another sub. time to wrap up and save the index.
					sub[k] = AllSubsGamble[i].ppn;
					k++;
				}//end new subelse
			} else {
		...
		for (i = 0; i < jsPsych.data.get().filter({test_part: 'SoloChoiceMarble'}).values().length; i++) {
			YourDecisions[i] = Number(jsPsych.data.get().filter({test_part: 'SoloChoiceMarble'}).values()[i].ChooseRisk) //Convert the boolean value to a number and then calculate the percentage
		} //end trialloop
		...
		while (!found){// loop thorugh it as long as you did not find a match ->what i could 
			...
			for (i = 0; i < subPercent.length; i++) {
				if ((subPercent[i] > (YourPercent + ProximityCrit))&&(subPercent[i] < (YourPercent + (ProximityCrit+0.02)))) {
					Index[count] = i;
					count++;
				} //end check proximity...
			} //end trialloop
			OneCloseSub = sub[jsPsych.randomization.sampleWithReplacement(Index, 1)]//sample randomly from the indices and by this pick a close subject
