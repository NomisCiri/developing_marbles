/**
 * jspsych-survey-text
 * a jspsych plugin for free response survey questions that throws an error if something that is not a number is 
 * displayed 
 * Josh de Leeuw
 *
...
jsPsych.plugins['survey-textNum'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
	  ...
	  function checkInp(x)// to see that there are only numbers in the array
	  {
	      var regex=/^[0-9]+$/;
	      if (!(x.match(regex)))
	      {
    ...
    display_element.querySelector('#jspsych-survey-text-next').addEventListener('click', function() {
	  ...
	  if (checkInp(val)){// only proceed if input is a number... 
      // save data
		  
      var trialdata = {
        "rt": response_time,
		  "age":question_data.Q0
	        };
