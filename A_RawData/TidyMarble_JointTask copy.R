...
rbind(
  tbl1%>%group_by(subject)%>%mutate(xi=row_number())%>%
    arrange(subject,xi)%>%filter(test_part!="PracticeTrials" & red_marbles!= "99")%>%
    mutate(PercentBlueEstimate2=lead(PercentBlueEstimate))%>%mutate(
      PercentBlueEstimate3=case_when(PercentBlueEstimate2=="NULL" & blue_marbles!="NULL"~lead(PercentBlueEstimate2),
                                     TRUE~PercentBlueEstimate2)
    )%>%mutate(HowSure=lag(HowSure)) %>%
    na.omit() %>%dplyr::filter(test_part=="SoloChoice"),
  tbl1%>%group_by(subject)%>%mutate(xi=row_number())%>%
    arrange(subject,xi)%>%filter(test_part!="PracticeTrials" & red_marbles== "99")
)%>%arrange(subject,xi)->TidyMarbleSolo
...
rbind(
  tbl2%>%group_by(subject)%>%mutate(xi=row_number())%>%
    arrange(subject,xi)%>%filter(test_part!="PracticeTrials" & red_marbles!= "99")%>%
    mutate(PercentBlueEstimate2=lead(PercentBlueEstimate))%>%mutate(
      PercentBlueEstimate3=case_when(PercentBlueEstimate2=="NULL" & blue_marbles!="NULL"~lead(PercentBlueEstimate2),
                                     TRUE~PercentBlueEstimate2)
    )%>%mutate(HowSure=lag(HowSure)) %>%
    na.omit() %>%dplyr::filter(test_part=="SocialChoice"),
  tbl2%>%group_by(subject)%>%mutate(xi=row_number())%>%
    arrange(subject,xi)%>%filter(test_part!="PracticeTrials" & red_marbles== "99" & test_part=="SocialChoice")
)%>%arrange(subject,xi)->TidyMarbleSocial

TidyMarble<-rbind(TidyMarbleSolo,TidyMarbleSocial)%>%arrange(subject,xi)%>%
  mutate(PercentBlueEstimate=PercentBlueEstimate3)%>%
  select(-PercentBlueEstimate2,-PercentBlueEstimate3)
...
TidyMarble<-TidyMarble%>%select(-xi)%>%unique()

trials<-rep(1,length(subs))

for(i in 1:length(subs)){
  trials[i]<-length(argh[argh$subject==subs[i],]$subject)
  print(trials[i])
}
...
write.csv(TidyMarble,file="A_RawData/TidyMarbleNewomg.csv")
...
MindInEyes<-merge(x = LucasMind, y = LucasMind2, by = "subject", all.x = TRUE)
...
TidyMarble%>%group_by(subject,age,OtherChoseRisk)%>%dplyr::summarize(
  MeanRisk=mean(as.numeric(as.character(ChooseRisk)))
)
